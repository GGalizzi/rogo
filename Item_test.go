...
func TestInventory(t *testing.T) {
	...
	g.area.items = append(g.area.items, NewItemFromFile("potion", 3, 3))
	g.player.Place(3, 3)
...
func TestKeys(t *testing.T) {
	...
	t.Logf("A key was created after placing a lockedDoor on the area. %+v links to %p.", validKey, tile)
	...
	if tile.locked || tile.blocks {
		t.Errorf("The key should have unlocked the tile. %+v points to %p: %+v", validKey, tile, tile)
	}
...
func TestPotion(t *testing.T) {
	...
	potion := NewItemFromFile("potion", 3, 3)
	potion.potency = 10
	g.player.inventory["potion"] = potion
